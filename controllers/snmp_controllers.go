...
func GetCpuUsageController(c *gin.Context) {
	...
	data := []MemSnmpAll{
		{Server: config.Server244, Value: (user[0] + sys[0] + nice[0] + irq[0] + soft[0] + rawW[0] + rawK[0]) - idle[0]},
		{Server: config.Server245, Value: (user[1] + sys[1] + nice[1] + irq[1] + soft[1] + rawW[1] + rawK[1]) - idle[1]},
		{Server: config.Server246, Value: (user[2] + sys[2] + nice[2] + irq[2] + soft[2] + rawW[2] + rawK[2]) - idle[2]},
		{Server: config.Server250, Value: (user[3] + sys[3] + nice[3] + irq[3] + soft[3] + rawW[3] + rawK[3]) - idle[3]},
		{Server: config.Server251, Value: (user[4] + sys[4] + nice[4] + irq[4] + soft[4] + rawW[4] + rawK[4]) - idle[4]},
		{Server: config.Server252, Value: (user[5] + sys[5] + nice[5] + irq[5] + soft[5] + rawW[5] + rawK[5]) - idle[5]},
	}
	c.JSON(200, data)
}
